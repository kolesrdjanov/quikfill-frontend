...
| **Host permission** `https://api.quikfill.io/*`             | The background service worker calls the QuikFill backend for user-initiated sign-in, settings sync, and AI assistance.                                                                                                                                                                                                       |
| **Broad host access** (content script matches `<all_urls>`) | QuikFill is a general-purpose form filler that must work on whatever site the user chooses. The content script runs on all sites so the user can scan and fill a form wherever they are. It acts only on explicit user action and sends redacted field summaries — never full page content or field values — to the backend. |
