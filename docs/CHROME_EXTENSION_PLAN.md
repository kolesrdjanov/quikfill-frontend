...
¹ Side panel & options were later retired to `apps/chrome-extension/legacy/` in v2.
...
> content overlay. The icon opens the popup (`default_popup`, auto-generated by
> WXT); only the `sidePanel` permission is dropped — `activeTab` is **retained**
> (granted on the popup-open gesture) and powers the per-site activation toggle
> that reads the active tab's hostname. The side-panel and options/settings
...
> ⚠️ **Superseded.** The v2 / flow-revamp notes earlier in this doc (the **toolbar
> popup** + **in-page Shadow-DOM overlay** design — no side panel, no `sidePanel`
> permission) are the **current** architecture. Everything below — `side_panel` as
> the primary UI, the `sidePanel` permission, the popup as a thin launcher, the
> options page — is the **original pre-revamp target**, kept for history. Read it as
> background, not as the live design. For the live fill flow see
> [`CHROME_EXTENSION_FLOW.md`](./CHROME_EXTENSION_FLOW.md).
